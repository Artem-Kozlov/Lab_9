public class WordCounter {
    ...
    public static int count(char[][] matrix, String word) {
        ...
        if (wordChars.length == 1)
            return countCharOccurrences(matrix, wordChars[0]);
    ...
    private static int right(char[][] matrix, char[] word, int[] startPos) {
        int j = 1;
        if(matrix[startPos[0]].length - startPos[1] < word.length)
            return 0;
        for (int i = startPos[1]+1; i < matrix[startPos[0]].length; i++) {
            if (j >= word.length)
                break;
            if (word[j] != matrix[startPos[0]][i]) {
                return 0;
            }
            j++;
        }
    ...
    private static int left(char[][] matrix, char[] word, int[] startPos) {
        int j = 1;
        if((startPos[1] + 1 < word.length))
            return 0;
        for (int i = startPos[1]-1; i >= 0; i--) {
            if (j >= word.length)
                break;
            if (word[j] != matrix[startPos[0]][i]) {
                return 0;
    ...
    private static int top(char[][] matrix, char[] word, int[] startPos) {
        int j = 1;
        if((startPos[0] + 1 < word.length))
            return 0;
        for (int i = startPos[0]-1; i >= 0; i--) {
            if (j >= word.length)
                break;
            if (word[j] != matrix[i][startPos[1]]) {
                return 0;
            }
            j++;
        }
    ...
    private static int bottom(char[][] matrix, char[] word, int[] startPos) {
        int j = 1;
        if(matrix.length - startPos[0] < word.length)
            return 0;
    ...
    private static int leftTop(char[][] matrix, char[] word, int[] startPos) {
        if((startPos[0] + 1 < word.length || startPos[1] + 1 < word.length))
            return 0;
        int j = 1;
    ...
    private static int rightTop(char[][] matrix, char[] word, int[] startPos) {
        if((matrix[startPos[0]].length - startPos[1] < word.length || startPos[0] + 1 < word.length))
            return 0;
        int j = 1;
        int row = startPos[0]-1;
        int column = startPos[1]+1;
    ...
    private static int leftBottom(char[][] matrix, char[] word, int[] startPos) {
        if((startPos[1] + 1 < word.length || matrix.length - startPos[0] < word.length))
            return 0;
        int j = 1;
        int row = startPos[0]+1;
        int column = startPos[1]-1;
